...
# Furniture colors — mirrors show_furniture.py FURNITURE list (lowercase keys)
FURNITURE_COLORS: Dict[str, str] = {
    "bed":             "#ff00ff",   # fuchsia   (bedroom)
    "bedside table":   "#00bfff",   # deepskyblue
    "wardrobe":        "#ffff00",   # yellow
    "sofa":            "#808000",   # olive     (living)
    "coffee table":    "#00fa9a",   # mediumspringgreen
    "tv unit":         "#dda0dd",   # plum
    "counter":         "#00ffff",   # aqua      (kitchen)
    "fridge":          "#7fff00",   # chartreuse
    "stove":           "#fa8072",   # salmon
    "kitchen sink":    "#7f007f",   # purple2
    "dining table":    "#ff1493",   # deeppink  (dining)
    "dining chair":    "#ffdead",   # navajowhite
    "toilet":          "#ff0000",   # red       (bathroom)
    "bathroom sink":   "#4169e1",   # royalblue
    "shower":          "#2e8b57",   # seagreen
    "bathtub":         "#ff8c00",   # darkorange
    "shoe rack":       "#696969",   # dimgray   (hallway)
    "console":         "#0000ff",   # blue
}
...
def _preprocess_plan(plan: Dict[str, Any]) -> Dict[str, Any]:
    """Convert GeoJSON dict values in a plan to shapely geometries (needed for plot_plan)."""
    from shapely.geometry import shape as shp_shape
    result = dict(plan)
    geom_keys = [
        "living", "bedroom", "bathroom", "kitchen", "door", "window",
        "wall", "front_door", "balcony", "storage", "veranda", "garden",
        "stair", "inner",
    ]
    for k in geom_keys:
        v = result.get(k)
        if isinstance(v, dict):
            try:
                result[k] = shp_shape(v)
            except Exception:
                pass
    return result


def plot_plan_furnished(plan: Dict[str, Any],
                        furniture: Dict[str, list],
                        scale: float,
                        ax: Optional[plt.Axes] = None,
                        legend: bool = True,
                        title: Optional[str] = None) -> plt.Axes:
    """
    Plot a floor plan with furniture overlaid.

    Args:
        plan:      Plan dict (may contain GeoJSON dicts or shapely geometries).
        furniture: Dict mapping room_type → list of furniture items.
                   Each item must have x_coord, y_coord, width_coord, height_coord.
        scale:     Metres per coordinate unit (used for context only; not needed for plot).
    ...
    processed = _preprocess_plan(plan)
    ax = plot_plan(processed, ax=ax, legend=legend, title=title, tight=False)

    added_types: list = []
    for room_type, items in furniture.items():
        for item in items:
            ...
            rect = Rectangle(
                ...
                linewidth=0.4, alpha=0.75, zorder=10,
            )
            ax.add_patch(rect)
            ax.text(
                x_c, y_c, ftype,
                ha="center", va="center", fontsize=5, zorder=11,
                bbox=dict(boxstyle="round,pad=0.1", fc="white", ec="none", alpha=0.5),
            )
            if ftype not in added_types:
                added_types.append(ftype)

    if legend and added_types:
        existing = ax.get_legend()
        existing_handles = existing.legend_handles if existing else []
        furn_handles = [
            Patch(facecolor=FURNITURE_COLORS.get(ft, "#cccccc"),
                  edgecolor="black", label=ft)
            for ft in added_types
        ]
        ax.legend(
            handles=list(existing_handles) + furn_handles,
            loc="upper left", bbox_to_anchor=(1, 1), frameon=False, fontsize=7,
        )
