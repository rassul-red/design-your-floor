...
log = logging.getLogger("locate_furniture")
...
def process_room(room_label: str, json_path: str, img_path: str,
    ...
    """
    log.info("[%s] Loading room JSON: %s", room_label, json_path)
    ...
    log.info("[%s] type=%s  area=%.1f m²  scale=%.4f", room_label, room_type, area_m2, scale)
    ...
    with open(img_path, "rb") as f:
        img_bytes = f.read()
    log.info("[%s] Image loaded (%.1f KB), calling Gemini model=%s ...", room_label, len(img_bytes) / 1024, model)
    ...
    try:
        ...
        log.info("[%s] Gemini responded in %.1fs (%d chars)", room_label, elapsed, len(raw_text))
        ...
        elapsed = time.time() - t0
        log.error("[%s] Gemini call FAILED after %.1fs — %s", room_label, elapsed, exc)
        return []
...
def main():
    ...
    log.info("Dispatching %d room(s) to Gemini (%s) in parallel...", len(jobs), model)
    ...
    with ThreadPoolExecutor(max_workers=len(jobs)) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                log.info("[%s] DONE — %d furniture item(s)", room_label, len(items))
            except Exception as exc:
                log.error("[%s] ERROR: %s", room_label, exc)
                furniture_by_room[room_label] = []

    log.info("All Gemini calls finished in %.1fs", time.time() - t_all)

    # ── save per-room furnished JSONs + PNGs ─────────────────────────────────
    log.info("Saving per-room furnished JSONs + PNGs...")
    for room_label, json_path in sorted(room_jsons.items()):
        with open(json_path) as f:
            room_json = json.load(f)
    ...
    if not os.path.isfile(plan_path):
        ...
        log.info("Full furnished plan JSON saved → %s", out_full_json)
